...
package deltax.gui.component {
    ...
    public class DeltaXEdit extends DeltaXScrollPane {
		 ...
		 * 滚动条拖动 
		 */		
        private function textChangeHandler(event:DXWndEvent):void{
            if (!this.digitOnly){
		 ...
		 * 
		 */		
        public function set maxTextLen(value:uint):void{
            if (this.m_maxTextLen != value){
                this.m_maxTextLen = value;
		 ...
		 * @param value
		 */		
        public function set maxDigit(value:Number):void{
            this.m_maxDigital = value;
        }
		 ...
		 * @param value
		 */	
        public function set minDigit(value:Number):void{
            this.m_minDigital = value;
        }
		 ...
		 * @param value
		 */
        public function set digitOnly(value:Boolean):void{
            if (value){
		 ...
		 * @param value
		 */
        public function set clipboardEnable(value:Boolean):void{
            if (value){
        ...
        private function _onFocus(event:DXWndEvent):void{
            var _local2:Boolean;
            var _local3:Boolean;
            if (((Capabilities.hasIME) && ((event.target == this)))){
                _local2 = (event.param as Boolean);
		 ...
		 * @param	value	文本
		 */
        private function insertStr(value:String):void{
            this.eraseSelected(false);
            this.m_insertString = value;
            if (value){
                if (this.multiline){
                    value = value.replace(/\r\n/g, "\n").replace(/\r/g, "\n");
                } else {
                ...
                if (value){
                    m_text = ((m_text.substr(0, this.m_selectBegin.x) + value) + m_text.substr(this.m_selectBegin.x));
                    this.m_selectBegin.x = (this.m_selectBegin.x + value.length);
                    if (hasEventListener(DXWndEvent.STATE_CHANGED)){
                        dispatchEvent(new DXWndEvent(DXWndEvent.STATE_CHANGED));
                    };
                };
		 ...
		 * @param	valueY	选择结束索引
		 */
        public function setSelection(valueX:int, valueY:int):void{
            valueX = MathUtl.limit(valueX, 0, m_text.length);
            if (valueY < 0){
                valueY = m_text.length;
            };
            valueX = MathUtl.limit(valueX, 0, m_text.length);
            valueY = MathUtl.limit(valueY, 0, m_text.length);
            this.m_selectBegin.y = this.getLineFromPos(valueX);
            this.m_selectBegin.x = valueX;
            this.m_selectEnd.x = valueY;
            this.setCurShow();
        }
		 ...
		 * 文本输入处理<br>
		 */
        override protected function onText(value:String):void{
            this.m_showTime = 0;
            if (!this.editable){
                return;
            };
