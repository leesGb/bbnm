...
package deltax.gui.base {
    ...
    public class DisplayImageInfo {
		 ...
		 * @param data
		 */		
		public function createPixelArea(data:ByteArray):void
		{
			if(this.texture)
			{
				var bitmapData:ByteArray = texture.bitmapData as ByteArray;
				if(bitmapData)
				{
					data.position = 0;
					bitmapData.endian = Endian.LITTLE_ENDIAN;
					bitmapData.position = 0;
					var i:uint = textureRect.x;
					var j:uint = textureRect.y;
					var xW:uint = textureRect.right;
					var yH:uint = textureRect.bottom;
					var color:uint;
					for(;j<yH;j++)
					{
						i = textureRect.x;
						for(;i<xW;i++)
						{
							bitmapData.position =((j*this.texture.width)+i)*4;
							color = bitmapData.readUnsignedInt();
							if(((color>>24)&0xff)>0)
							{
								data.writeByte(1);
							}else
